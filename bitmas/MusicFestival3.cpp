...
int pd(int stages_state, int atual_time, map<int, show>::iterator pointer){
	...
	for(;pointer != shows.end(); pointer++){
		stages_state_aux = stages_state | (1<<(pointer->second.stage - 1));
		memo[stages_state][atual_time]  = max(memo[stages_state][atual_time],
								 pd(stages_state_aux, pointer->second.end, shows.find(pointer->second.start)) + pointer->second.gold);
	}
...
int main(){
	...
	int resposta = pd(0, 0, shows.begin());
	if(resposta > 0)
		cout << resposta << endl;
	else
		cout << "-1" << endl;
